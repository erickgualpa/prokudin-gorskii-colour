...
working_image = 2 # De la 0 a la 4
mode = "phase" # "fft", "phase", "norm", "edges", None=(Convolución espacio)
...
if mode == "fft":
	...
	cv2.imwrite(img_path + "im"+str(working_image+1)+"/product_espacio_fourier.jpg", merged)
	#####################################################################
elif mode == "phase":
	## 5 - CORRELACIÓN (De fase basada en Fourier) ######################
	merged = alignByConvolution(ch, mode="phase")
	cv2.imwrite(img_path + "im"+str(working_image+1)+"/correlacion_fase_fourier.jpg", merged)
	#####################################################################
elif mode == "norm":
	## 6 - CORRELACIÓN (Normalizada) ####################################
	merged = alignByConvolution(ch, mode="norm")
	cv2.imwrite(img_path + "im"+str(working_image+1)+"/correlacion_normalizada.jpg", merged)
	#####################################################################
elif mode == "edges":
	## 7 - CORRELACIÓN (Edges) ##########################################
	merged = alignByConvolution(ch, mode="edges")
	cv2.imwrite(img_path + "im"+str(working_image+1)+"/correlacion_edges.jpg", merged)
	#####################################################################
else: 
	## 3 - CORRELACIÓN (Convolución del espacio) ######################## 
	merged = alignByConvolution(ch)
	cv2.imwrite(img_path + "im"+str(working_image+1)+"/convolucio_espai.jpg", merged)
	#####################################################################
